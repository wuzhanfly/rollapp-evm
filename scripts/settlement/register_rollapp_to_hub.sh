...
set -x
"$SETTLEMENT_EXECUTABLE" tx rollapp create-rollapp "$ROLLAPP_CHAIN_ID" "$ROLLAPP_ALIAS" EVM --bech32-prefix "$BECH32_PREFIX" \
  --init-sequencer "$SEQUENCER_ADDR" --genesis-checksum "$GENESIS_HASH" --metadata "$METADATA_PATH" \
	--from "$DEPLOYER" \
	--keyring-backend test \
  --gas auto --gas-adjustment 1.2 \
	--fees 1dym
